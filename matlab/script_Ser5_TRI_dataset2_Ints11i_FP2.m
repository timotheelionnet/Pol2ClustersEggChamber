...
% add path to MIJ jars for image J visualizations (cheeck your own path)
javaaddpath '/Applications/MATLAB_R2022a.app/java/jar/mij.jar'
javaaddpath '/Applications/MATLAB_R2022a.app/java/jar/ij.jar'
...
%% Load images of representative nuclei in ImageJ

imageJViz = 1; % set to zero to skip ImageJ

metricChIdx = 4;
metricRange = 10.^[0.2,0.3]; % G phase range
metricRawOrPlasmNorm = 'plasmNorm';
removeBackgroundEggChambers = 1;
idxData0 = ec.nucFullT.eggChamber_Idx ~=0;
nNucleiToLoad = 5;

metricVarName = ['nucAvgClust',strrep(largeClusterMetricName,'.','pt'),...
    '_clust_C',num2str(metricChIdx),'Median_nucleoliSubtracted'];

if imageJViz
    addpath ''
    MIJ.start;

    % load images from condition 1
    condIDs = 1;
    loadNucleiImagesWithinMetricRange(ec,fijiOutFolder,idxData0,condIDs,metricVarName,...
        metricChIdx,metricRange,metricRawOrPlasmNorm,nNucleiToLoad,...
        largeClusterMetricName,smallClusterMetricName,channelNamesForDisplays);
    
    % load images from condition 3
    condIDs = 3;
    loadNucleiImagesWithinMetricRange(ec,fijiOutFolder,idxData0,condIDs,metricVarName,...
        metricChIdx,metricRange,metricRawOrPlasmNorm,nNucleiToLoad,...
        largeClusterMetricName,smallClusterMetricName,channelNamesForDisplays);
...
function loadNucleiImagesWithinMetricRange(ec,fijiOutFolder,idxData0,condIDs,metricVarName,...
    channelIdx,metricRange,metricRawOrPlasmNorm,nNucleiToLoad,...
    largeClusterMetricName,smallClusterMetricName,channelNamesForDisplays)
    ...
    switch metricRawOrPlasmNorm
        ...
        case 'plasmNorm'
            [~,~,baseName,suffix] = ec.splitVarName(metricVarName);
            if contains(metricVarName,['C',num2str(channelIdx)])
                varType = 'channel';
            else
                varType = 'geom';
            end
            plasmVarName = ec.buildVarName('plasm',channelIdx,baseName,suffix,varType);
            yData = ec.nucFullT.(metricVarName)./ec.nucFullT.(plasmVarName);
    end
    
    loadNucleiImagesWithinArbitraryMetricRange(ec,fijiOutFolder,idxData0,condIDs,yData,metricRange,...
        nNucleiToLoad,largeClusterMetricName,smallClusterMetricName,channelNamesForDisplays);
end

%% 
function loadNucleiImagesWithinArbitraryMetricRange(ec,fijiOutFolder,idxData0,condIDs,yData,metricRange,...
    nNucleiToLoad,largeClusterMetricName,smallClusterMetricName,channelNamesForDisplays)

    % select nuclei from desired condition
    idxCond = ismember(ec.nucFullT.cond_Idx,condIDs);
    
    % list nuclei rows that fit in a range of a nucleus metric
    idxData = idxData0 & idxCond ...
        & yData >= metricRange(1) & yData <= metricRange(2);
    
    % generate basic variables for each loaded nuclei for rapid diagnosis
    ...
    if nNucleiToLoad <= size(nucIdxList,1)
        disp(['Loading ',num2str(nNucleiToLoad),' randomly picked from the ',...
            num2str(size(nucIdxList,1)),' nuclei in the condition']);
        p = randperm(size(nucIdxList,1));
        idxToLoad = p(1:nNucleiToLoad);
    else
        disp(['Cannot load ',num2str(nNucleiToLoad),' nuclei because there are only ',...
            num2str(size(nucIdxList,1)),' nculei in this condition.']);
        idxToLoad = 1:size(nucIdxList,1);
    end
    ...
    for i=1:numel(idxToLoad)
        ...
        [~,f,~] = fileparts(nucIdxList.sample_InputFileName(idxToLoad(i)));
        nucFileName = fullfile(fijiOutFolder,...
            ec.conditionNames{nucIdxList.cond_Idx(idxToLoad(i))},...
            f,'nucTIF',['nuc',num2str(nucIdxList.nuc_Label(idxToLoad(i))),'_masks.tif']);
        disp(['img',num2str(i),': condition ',num2str(nucIdxList.cond_Idx(idxToLoad(i))),...
            '; sample ',num2str(nucIdxList.sample_Idx(idxToLoad(i))),...
            '; nuc',num2str(nucIdxList.nuc_Label(idxToLoad(i)))]);
        disp(['    loading ',nucFileName,'...']);
        disp(['    row: ',num2str(idxToLoad(i)),'; nClusters: ',num2str(nucIdxList.(['nuc_NumClusters',strrep(largeClusterMetricName,'.','pt')])(idxToLoad(i)))]);
        for j=1:numel(channelNamesForDisplays)
            metricVarName = ['nucAvgClust',strrep(largeClusterMetricName,'.','pt'),'_clust_C',num2str(j),'Median_nucleoliSubtracted_plasmCorr'];
            disp(['    Channel ',num2str(j),'; cluster int plasmNorm: ',num2str(nucIdxList.(metricVarName)(idxToLoad(i)))]);
        end
        % loadImage
