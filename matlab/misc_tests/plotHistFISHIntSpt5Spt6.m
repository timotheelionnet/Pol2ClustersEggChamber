...
condNames = {'mCherry RNAi','Spt6 RNAi','Spt5 RNAi'}; 
...
alpha(0.25);
xlim([-0.5,numel(condVarsCorr)]);
xticks( 0.25 + (0:(numel(condVarsCorr)-1)));
...
%----------------------------------------------------------------------------------
% the following fit parameters might need to be adjusted to match different experiments
histLogBin = 0.1; % bin size for the histogram of the log transformed variable
a1min = 0.1; % minimum fraction of low-intensity population (0-1)
a1max = 0.9; % maximum fraction of low-intensity population (0-1)
mu1min = 1; % minimum value of the mean of the low-intensity population in the log-transformed variable
...
for i=1:numel(condVarsCorr)
    ...
    % plot fit components
    plot(ah{i},xfit{i},yfit1{i},'Color','k','DisplayName','fit component 1');
    plot(ah{i},xfit{i},yfit2{i},'Color','k','DisplayName','fit component 2');
