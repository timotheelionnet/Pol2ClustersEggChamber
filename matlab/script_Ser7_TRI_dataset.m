...
% fijiOutFolder = '\\research-cifs.nyumc.org\Research\lionnt01lab\lionnt01labspace\Feiyue_Tim\TRI-new-dataset-with-Ser7ph-using-MPM2-and-pol2-channels';
fijiOutFolder = '\Volumes\lionnt01lab\lionnt01labspace\Feiyue_Tim\TRI-new-dataset-with-Ser7ph-using-MPM2-and-pol2-channels';

% entere here the path where to save the data output by Matlab
% matlabOutFolder = '\\research-cifs.nyumc.org\Research\lionnt01lab\lionnt01labspace\Feiyue_Tim\TRI-Ser7ph-matlabOut';
matlabOutFolder = '\Volumes\lionnt01lab\lionnt01labspace\Feiyue_Tim\TRI-Ser7ph-matlabOut2';
...
channelNamesForDisplays = {'DAPI','MPM2','Ser7ph','PolII'};
...
fijiOutFolder = strrep(fijiOutFolder,'/',filesep);
fijiOutFolder = strrep(fijiOutFolder,'\',filesep);

matlabOutFolder = strrep(matlabOutFolder,'/',filesep);
matlabOutFolder = strrep(matlabOutFolder,'\',filesep);
...
% make current dir to save to (some saveas functions give weird permission
% issues on the server)
if ~exist('tmpJunk','dir')
    mkdir('tmpJunk');
end
...
% all stages
[fullTable,avgEcTable,avgCondTable,fh] = ec.scatterPlotAndSaveNucTableMetricByEggChamber( ...
...
for i=1:numel(channelNamesForDisplays)
    [nucTable,avgEcTable,avgCondTable,fh] = ec.scatterPlotAndSaveNucTableMetricByEggChamber( ...
        'nuc',i,'Mean','raw',...
...
for i=1:numel(channelNamesForDisplays)
    ...
    legend show
    xlabel(['Cluster ',channelNamesForDisplays{i},' intensity, bg-corr']);
    ylabel('Count');
...
legend show
xlabel(['clustInt',channelNamesForDisplays{chX},'-',channelNamesForDisplays{chY},' ratio plasmNorm']);
ylabel('Count');
...
legend show
xlabel(['smallClustInt',channelNamesForDisplays{chX},'-',channelNamesForDisplays{chY},' ratio plasmNorm']);
ylabel('Count');
xlim([0 10]);
...
%% plotting MPM2+ and MPM2- HLB metrics side by side
chRef = 2; % channel used to stratify the data in positive vs negative
...
%% plotting MPM2+ and MPM2- Ser5ph/Pol II side by side
