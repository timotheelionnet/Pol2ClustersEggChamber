macro "test Nuclei Segmentation"{
	...
	outFolder = "/Users/lionnt01/Documents/data/feiyue/egg chamber image quant/testOut/";
	
	// collect files in input dir and its subfolders
	...
	imgNames = newArray("test1","test2","test3","test4","test5");
	...
	for (i = 0; i < fileList.length; i++) {
		...
		// save metrics
		runMetricsAndSave(outName2,outFolder+outSubDirList[i]+fileList[i]+"metrics.csv",fileList[i]);
		...
		// merge result w original and save
...
function runMetricsAndSave(inputImgTitle,savePath,inputFileName){
...
function segmentNuclei3D(originalImgTitle,outputImgTitle,nucleiChannel,avgNucleiDiameterInUm){
	...
	//ratio of the size of the minimal object to be retained after the first filtering step.
	minVolFactor = 36;
	...
	minVolInVoxels = round(4*3.14/3* pow(avgNucleiDiameterInUm,3)/(dX*dY*dZ)/minVolFactor);
